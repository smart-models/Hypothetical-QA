...
def test_qa_generation_basic_functionality(client):
    ...
    print(f"Successfully generated {len(chunks)} Q&A pairs from "
...
def test_custom_prompt_functionality(client):
    ...
    custom_prompt = ("Generate exactly 2 educational questions and answers "
...
def test_qa_format_validation(client):
    ...
    for qa_item in data["chunks"]:
        ...
        answer_indicators = [
            "answer:", "a:", "ans:", "answer is", "the answer", 
            "response:", "reply:", "solution:", "explanation:"
        ]
        question_indicators = [
            "question:", "q:", "que:", "query:", "what", "how", 
            "why", "when", "where"
        ]
        
        # Must have either answer indicators OR question indicators
        # to be considered Q&A format
        has_answer_indicators = any(keyword in text_lower 
                                    for keyword in answer_indicators)
        has_question_indicators = any(keyword in text_lower 
                                      for keyword in question_indicators)
...
def test_quality_scoring_functionality(client):
    ...
    assert 0.0 <= quality_score <= 1.0, \
        f"Quality score {quality_score} out of range"
